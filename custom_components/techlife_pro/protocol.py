...
class TechLifeProtocol:
    ...
    def parse_status(payload: bytes) -> Optional[TechLifeState]:
        ...
        Valid status frames start with 0x11 and have 0x22 at byte 22.
        """
        if not payload or len(payload) < 23:
            return None
        if payload[0] != 0x11 or payload[22] != 0x22:
            return None

        msg_hex = payload.hex()
        state = TechLifeState()

        # byte 12: 0x00=RGB strip, 0x01=white-only.
        if payload[12] == 0x00:
            state.light_type = LIGHT_TYPE_RGB
        elif payload[12] == 0x01:
            state.light_type = LIGHT_TYPE_WHITE

        # byte 19 (hex chars 38..40): 0x23=on, 0x24=off.
        state_hex = msg_hex[38:40]
        if state_hex == "23":
            state.is_on = True
